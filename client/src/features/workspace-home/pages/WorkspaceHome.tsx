...
export default function WorkspaceHome() {
  ...
  const {
    ...
    error: errorDocs,
    refetch: refetchDocs,
  } = useWorkspaceRecentDocuments(workspaceId || "")
    ...
    <div className="p-6 max-w-6xl mx-auto space-y-6">
      ...
      {errorSummary && (
        <div className="p-4 border border-destructive/20 bg-destructive/5 rounded-lg flex items-center justify-between gap-4 mb-8 text-left">
          ...
          <Button size="xs" variant="outline" onClick={() => refetchSummary()} className="cursor-pointer">
            Retry
          </Button>
        </div>
      )}
        ...
        <div className="lg:col-span-2 space-y-3">
          ...
          {errorDocs && (
            <div className="p-4 border border-destructive/20 bg-destructive/5 rounded-lg flex items-center justify-between gap-4 text-left">
              ...
              <Button size="xs" variant="outline" onClick={() => refetchDocs()} className="cursor-pointer">
                Retry
              </Button>
            </div>
          )}

          {!loadingDocs && !errorDocs && (!recentDocs || recentDocs.length === 0) && (
        ...
        <div className="lg:col-span-2 space-y-3">
          ...
          {errorActivities && (
            <div className="p-4 border border-destructive/20 bg-destructive/5 rounded-lg flex items-center justify-between gap-4 text-left">
              ...
              <Button size="xs" variant="outline" onClick={() => refetchActivities()} className="cursor-pointer">
                Retry
              </Button>
            </div>
          )}

          {!loadingActivities && !errorActivities && (!activities || activities.length === 0) && (
        ...
        <div className="lg:col-span-1 space-y-3">
          ...
          {errorMembers && (
            <div className="p-4 border border-destructive/20 bg-destructive/5 rounded-lg flex items-center justify-between gap-4 text-left">
              ...
              <Button size="xs" variant="outline" onClick={() => refetchMembers()} className="cursor-pointer">
                Retry
              </Button>
            </div>
          )}

          {!loadingMembers && !errorMembers && (!members || members.length === 0) && (
            <Card className="border border-border">
              <CardHeader className="pb-3 text-left">
          ...
          {!loadingMembers && !errorMembers && members && members.length > 0 && (
            <Card className="border border-border">
              ...
              <CardContent className="space-y-3 pt-1 text-left border-t border-border/50 pt-3">
                {members.map((member) => (
